...
var managementEndpoints = []managementEndpoint{
	...
	{http.MethodGet, "/traffic-capture", "Poll captured requests of the watched account and renew the listener", (*App).getTrafficCapture},
	{http.MethodGet, "/traffic-capture/entry", "View one captured request and response with redacted headers", (*App).getTrafficCaptureEntry},
	{http.MethodPost, "/traffic-capture/watch", "Start listening to one upstream account in memory", (*App).watchTrafficCapture},
	{http.MethodDelete, "/traffic-capture/watch", "Stop listening to upstream traffic", (*App).stopTrafficCapture},
	{http.MethodDelete, "/traffic-capture", "Clear captured requests from memory", (*App).clearTrafficCapture},
	{http.MethodPut, "/traffic-capture/settings", "Save whether response hooks are registered for capture", (*App).setTrafficCaptureSettings},
