...
func TestV15ForwardedForBlockMigrationPreservesDataAndRollsBack(t *testing.T) {
	for _, conflict := range []bool{false, true} {
		t.Run(map[bool]string{false: "migrate", true: "rollback"}[conflict], func(t *testing.T) {
			...
			defer raw.Close()
			oldSchema := strings.TrimSuffix(strings.TrimSuffix(strings.TrimSuffix(strings.TrimSuffix(schema, groupRoutingModeSchema), groupDisabledSchema), groupRuleSchema), forwardedForBlockSchema)
