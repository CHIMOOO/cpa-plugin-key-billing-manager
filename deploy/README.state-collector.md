...
请先切换到本次发布的源码标签 `v0.1.7`，确保构建内容与插件一致。源码仓库中的 `compose.state-collector.yaml` 是 Linux 主机网络示例。如果 CPA 端口发布在宿主的 8317，可直接使用 `http://127.0.0.1:8317`。采集器和 CPA 在不同容器网络时，`127.0.0.1` 不会指向 CPA；应加入 CPA 的现有 Docker 网络，移除 `network_mode: host`，使用对应服务名（例如 `http://cpa:8317`）。
...
export CPA_COLLECTOR_VERSION=0.1.7
...
Use the included systemd unit for boot-time startup, or build the Docker Compose example from tag v0.1.7 with CPA_COLLECTOR_VERSION=0.1.7 for a sidecar container. systemd 247+ is required for `LoadCredential`; restart the unit after rotating its password file. The container runs as UID 65532, so its mounted password file must be readable by that UID. Set the CPA origin to an address reachable from the collector, and preserve normal HTTPS verification.
