...
选择探测账号、填写模型后，可开始探测并保持页面打开以连续采集。**已停用的 Codex OAuth 账号也可以选中并采集**，列表会标注停用状态；采集不会启用账号，日常业务仍遵守 CPA 的停用设置。账号需要有效的 OAuth 凭证；过期凭证需重新登录或由 CPA 刷新。已删除或不再是 Codex OAuth 的账号不参与探测，已保存的失效选择可在页面中移除。静态代理一行对应一个固定出口，同一账号/模型失败后冷却 55 分钟；轮换代理同一 URL 最多连续尝试 10 次，然后冷却 10 分钟。成功后按模板实际签发时间和有效期安排续采，缩短有效期也能及时刷新。代理支持带账号密码的 HTTP、HTTPS、SOCKS5、SOCKS5H URL。代理输入留空表示保留已保存值，清空池需使用明确的清空操作。状态接口只返回代理数量，探测结果中的地址会脱敏。

静态池和轮换池各支持最多 20,000 条代理，完整配置上限为 16 MiB。管理页保存大配置时自动分块上传，每次请求约 16 KiB，避免大量代理一次性提交触发常见网关的 `413 Request Entity Too Large`。全部上传并校验通过后才一次性生效；中途失败保留现有配置和页面草稿，可直接重试。上传期间若其他页面修改了配置，本次提交会被拒绝，请刷新后重新保存。未完成上传仅保留在内存，15 分钟无活动后在后续调用中清理，不写入状态文件。
