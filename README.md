...
| Codex Turn State | 按账号与模型采集、续期和注入模板；不同账号并行探测；有效桶保护；静态/轮换代理、10 并发连通性检测、可选自动淘汰；按需开启的诊断日志 |
| 模型测试 | 多账号并发诊断与连续多次测试；按账号汇总通过情况、声明模型是否一致和耗时；账号列表标出已挂代理，SVG 动画可预览，不执行生成代码 |
| 风控中心 | 本地关键词与模型范围规则、观察/请求前拒绝、命中内容哈希记忆；不保存提示词正文 |
| 流量抓包 | 同时监听多个上游账号并可逐个暂停，实时查看请求头、请求体、响应和上游响应模型；只在内存中保留，凭证与正文文本默认打码，可下载为文本 |
...
每个 Key 最多绑定一个启用的唯一组；新建、编辑、启用或批量绑定产生冲突时，整次操作会被拒绝，保留已有设置。禁用唯一组后，其余启用组按普通合并规则恢复参与。启用唯一组时，Key 单独配置的凭证白名单与「唯一＋通用」号池取交集，模型白名单同样取交集，拒绝项仍优先；单 Key 的规则不能扩大该号池。Key 从未配置直接路由时继承两组号池；直接配置里有任何条目时，只有直接规则或路由允许的凭证参与交集，仅模型或仅拒绝列表都不会授予凭证；完全为空的直接配置（「清空」）重新继承分组。要禁止某个 Key，请停用它或添加拒绝项。访问控制关闭时，这些分组限制也暂停。
...
逐账号并发上限为 0–1000，0 表示不限。账号页可勾选账号或点击「全选当前筛选账号」，填写统一并发上限后点击「应用到所选账号」，通过一次请求原子保存所选账号的设置。切换筛选保留已选项，页面显示隐藏的已选数量；关闭「仅启用」可选择停用账号，设置并发不会启用账号。未选择的账号保持原配置（凭证轮换后属于同一账号的别名仍共用上限）。

该上限控制**业务请求并发**，不改变 State 采集并发。它与下游 API Key 并发同时生效：流式请求持续占位，结束、断开或切换重试账号时释放原账号槽位。设置保存在 `<state_file>.account-runtime.json`，备份时一并保留。

## 模型测试

「模型测试」用于对比选定账号、模型对同一提示词的响应，并可对每个账号连续测试多次。它通过 **CPA 的管理接口发起诊断请求**，不经过完整的下游 API Key 业务链路，因此不能代替 Key 分组、路由与订阅额度的业务验收。测试会消耗上游额度；该调用路径不产生 `usage.handle`，不会写入下游用量与计费记录，页面也不把响应中的数字当作可靠的 Token、费用或 TTFT 指标。

模型测试需要 **CPA 7.3.4 兼容宿主及 Schema 6**。当前支持可精确确认身份的 **Codex OAuth 认证文件**，以及 CPA 原生配置中的 **OpenAI 兼容、Codex / Responses、Claude 和 Gemini API Key 通道**。配置型账号通过当次读取的原生管理配置、账号索引和精确指纹匹配；部分旧宿主不会把这些账号列入认证文件清单，因此不依赖该清单作为唯一来源。其他类型的 OAuth 文件（例如 Claude、Antigravity）、缺少或重复的账号索引，以及未支持的自定义请求头或协议会明确拒绝测试。**已停用账号也可单独诊断，测试不会修改其停用状态或让它重新参与业务轮询**；凭证仍须有效。账号接入模块发布的通道也需满足这些条件，不会直接使用保存凭证的辅助记录发请求。

页面自上而下分为测试设置、测试账号和回答对比三部分：

1. **测试设置**：填写实际上游模型 ID，选择题目，设置「每个账号测试次数」（1–100）和「并发请求数」（1–8）。同一批使用同一模型名和提示词；选中配置型通道时提供模型/别名候选，每次执行前重新读取其配置并解析别名。编辑预设提示词后自动按自定义提问处理，不套用原题判分。提示词上限为 8192 UTF-8 字节。
2. **测试账号**：按 **Codex、Claude、Gemini、OpenAI 兼容或其他服务商**筛选，也可搜索并多选，一批最多 50 个账号，请求总数（账号数 × 次数）最多 1000。**单独挂了代理的账号在卡片上显示「代理 主机:端口」**，列表上方说明未单独设置代理的账号走全局代理还是直连。已停用账号会标注状态并允许勾选；不支持测试或身份不明确的条目不展示。
3. **执行方式**：不同账号在并发上限内同时执行；**同一账号同一时间只发一个请求，多次测试依次执行**，既不超过账号自身的并发限制，也能看出连续表现。失败不自动重试。点击停止或离开模型测试页后不再开始新的请求，已发出的请求仍会收尾。队列由当前管理页面执行，刷新、退出登录或关闭页面不会在服务器上继续。插件同时最多保留 8 个进行中的测试。
4. **回答对比**：上方按账号汇总，一行一个账号：连续结果色块（绿色通过、红色失败或未通过、蓝色待人工检查、黄框表示声明模型与发送模型不一致，点击色块定位到那一次）、检查通过数、各声明模型及出现次数、平均耗时。下方是逐次结果表，可按账号或「失败或未通过」「声明模型不一致」筛选，每行显示状态、声明模型、耗时和回答最后一行，点击展开完整回答、判分、模型信息、代理通路和本次提示词。
5. 结果只保存在当前页面内存中，开始新的测试会替换旧结果，可手动清除。判分通过只说明本题答案符合要求，不能证明真实模型身份或整体能力，建议固定模型名与提示词，对照已知正常账号重复比较。

预制题来源与使用方式：

| 题目 | 来源与本项目处理 | 判断重点 |
| --- | --- | --- |
| 糖果保证问题 | [codex-candy-eval](https://github.com/haowang02/codex-candy-eval/blob/main/codex_candy_eval.py) 中的原题（三种口味、圆形和五角星形，按表格给出数量），要求回答最后一行写“最终答案：数字” | 自动检查最后一行的最终答案是否为 21；只在推理中出现 21 不算通过 |
| 鹈鹕骑自行车（SVG 动画） | [Simon Willison 原文](https://simonwillison.net/2024/Oct/25/pelicans-on-a-bicycle/)中的“鹈鹕骑自行车”，本项目改为 SVG 原生动画（车轮、脚踏转动，身体起伏） | 人工看长喙、喉囊、自行车结构、骑乘关系和动画是否流畅；页面直接播放动画，可重新播放 |
| 自定义提问 | 自由填写提示词 | 人工比较 |

这些是小型诊断题，不是正式跑分，也没有经验证的“降智阈值”。熟悉的固定题可能被模型记住；应结合多类题目和人工审查，不使用模型自报的名称、知识截止日期、juice 或回答长短判定身份与质量。SVG 以隔离图片方式预览：保留 SVG 原生动画，过滤脚本、事件属性、样式和外部资源，同时保留原始回答文本。回答中的代码不会执行。
...
「流量抓包」用于排查上游账号实际收到的请求。在账号列表中搜索并勾选一个或多个账号即开始监听，每个账号可单独暂停、继续或停止。页面每 1.5 秒拉取一次，列出被调度到这些账号的请求及所属账号、请求模型和上游响应里声明的模型；点开可看到路径、请求头、请求体，以及响应头和响应正文（流式响应按分块拼接实时增长）。请求、上游请求体和响应都可以复制或下载为 `.txt`，也可以下载包含全部内容的完整记录。重试到其他账号的请求会标注出来；如果新账号也在监听，重试会单独记为一条。

- **只在内存中**：不写磁盘、不进日志，最多保留 100 条，每个正文最多 2 MiB，总量超过 48 MiB 时淘汰最旧的记录；CPA 重启或点击清空即消失。
- **自动停止**：页面保持打开时持续监听；离开此页或关闭页面约 45 秒后停止捕获新请求，插件不运行任何后台定时器。
- **打码**：`Authorization`、`Cookie`、`X-Api-Key` 以及名称含 key、token、secret、password、signature 的请求头值会被替换为 `[redacted]`。页面上的请求体和响应体默认只保留协议结构（类型、角色、模型、ID 等），用户输入、输出、提示词和工具说明等文本显示为 `***`，下载的文件同样打码。插件内存中保存的仍是原文，请只在需要排查时开启。
- **能看到什么**：显示的是插件钩子在选定账号之后看到的内容，即客户端路径、请求头和请求体；开启响应钩子后，还能看到协议转换后发往上游的请求体。CPA 在上游执行器内部添加的上游凭证和最终 HTTP 报文不经过插件，这里看不到。
- **响应需要注册响应钩子**：只抓请求时不增加额外开销。CPA 只在加载或重新配置插件时读取钩子声明，所以切换「注册响应钩子以捕获响应」后，页面会自动把插件配置原样保存一次，让 CPA 重新注册插件（与在管理中心保存插件配置等效）；未生效时可点「重新加载插件使其生效」重试，或重启 CPA。开启后每个流式分块都会经过插件（未监听时只做一次快速判断就返回）。如果 Codex Turn State 已开启，响应钩子本来就已注册。该开关保存在 `<state_file>.traffic-capture.json`。
...
**加入采集列表的账号默认启用业务保护**：请求所选模型时，只允许使用该账号、实际模型对应的有效模板；同一账号的其他模型不经过 State。桶缺失或过期、注入关闭、观察模式以及 `replace-only` 无法替换当前请求时，均拒绝业务流量；其他账号仍按自身规则调度。该保护独立于采集开关，在页面顶部单独保存。切换为 `always` 可处理不携带模板的客户端请求，但仍需先有有效桶。旧宿主或缺少所选账号信息时会保守拒绝；宿主提供的 schema 6 是最低兼容标记，不能证明任意分支包含请求头转发修复。
...
| Cookie | 上游 Cookie 按账号只保存在内存。参考 `ccodex-rotate` 的冻结规则，默认只有带 292 的响应（探测或业务响应，不区分模型）才会刷新该账号的 Cookie，312 等其他响应保留上次 292 带来的那组 Cookie；开启「每个响应都刷新 Cookie」（`cookie_refresh_all`，默认关闭）后每个响应都会刷新。默认 240 秒内所选模型的请求都会带上，与是否注入模板无关 |
| Cookie 刷新轮次 | 列表中第一个模型采到 292 后，默认 30 秒（`cookie_refresh_seconds`）开始一轮刷新：先试上次返回 292 的出口，再逐个换下一个出口，直到拿到 292；每轮每个出口最多试一次、最多 25 个，冷却中的出口跳过。刷新遇到 312 不会让出口冷却，也不会自动丢弃代理；静态出口本身连接失败时仍按静态出口冷却。整轮没拿到 292 时保留当前模板，等待 `cookie_retry_seconds`（默认 300 秒，可设 30–3600）后开始下一轮。每次尝试都计入每小时上限 |
| 并发探测数 | `probe_parallel` 默认 3，可设 1–10。每个账号同一时间只探测一个桶，不同账号同时探测，该数字限制同时探测的账号数；狂暴续采窗口内的桶不受「每个账号一次一个」的限制，最多同时 10 个 |
...
「运行状态」页底部的「诊断日志」用于排查模板或 Cookie 用几分钟就失效的问题。它**默认不记录**，点击「开始记录」后才按账号写入：

- 开始时所选账号和模型现有的 292 模板与 Cookie 快照；
- 每次探测的来源（Cookie 刷新、续采或采集）、结果、响应 State 长度、状态码、出口和本轮刷新次数；
- Cookie 何时更新，以及哪些响应因冻结规则没有被采用；
- 业务响应学到模板、清空或丢弃模板；
- 所选账号和模型的每个业务请求：注入了哪张模板及其签发后秒数、是否带了 Cookie 及 Cookie 已存在的秒数、客户端自带的 State 长度，以及响应状态码和响应 State 长度。

对照面板上出错的时间，可以判断失效是跟请求次数有关，还是跟 292 或 Cookie 的时长有关。日志只记录模板指纹、Cookie 组指纹、Cookie 名称、长度和时长，不含 State 原文或 Cookie 值；出口地址隐藏代理用户名和密码。

日志只保存在 CPA 进程内存中，最多 20000 条，超出时先丢弃最旧的并显示丢弃条数；CPA 重启或切换数据路径会清空，普通设置保存不会。表格显示最新 100 条，记录期间页面每 3 秒刷新；「下载 CSV」导出全部记录（UTF-8 带 BOM）；「清空」需要确认，不改变记录开关；「停止记录」后已记录的内容仍保留。

### 第一次使用怎么操作

1. **选账号和模型。** 在「采集范围」勾选需要保护的 Codex OAuth 账号，填写实际要用的模型。账号需有有效登录凭证、模型权限和可用额度；State 不会补充已耗尽的账号额度。
2. **配置并检测代理。** 一行一个完整代理 URL，固定出口填静态池，每次连接更换出口的填轮换池。「测试连通性」只从 CPA 服务器连接代理网关的 TCP 端口，5 秒超时、最多 10 并发，不访问 OpenAI。通过只说明网关端口可达，不代表代理认证、实际出口或 State 采集成功。
...
自动淘汰会立即保存，并在日志中显示删除结果与剩余数量；保存失败会保留代理并继续探测。文本框会同步最新池，有未保存编辑时保留草稿并提示重新加载。它与连通性检测后的「移除本次失败代理」不同：后者只编辑草稿，仍需点击保存。开启淘汰前可先复制列表留存。

### 账号与模板

模板严格按 CPA 选中的账号和实际上游模型隔离。`always` 不会生成模板，也不会跨账号、模型使用；`pass` 可能表示请求已经带着当前模板，应结合界面中的具体原因判断。响应头长度是该第三方项目的识别规则，不代表对模型回答质量的独立验证。

安装独立服务器采集器后，选择探测账号、填写模型并点击开始，即可关闭页面持续采集。**已停用的 Codex OAuth 账号也可以选中并采集**，列表会标注停用状态；采集不会启用账号，日常业务仍遵守 CPA 的停用设置。账号需要有效的 OAuth 凭证；过期凭证需重新登录或由 CPA 刷新。已删除或不再是 Codex OAuth 的账号不参与探测，已保存的失效选择可在页面中移除。静态代理一行对应一个固定出口，同一账号/模型失败后默认冷却 55 分钟；每个账号、模型的整个轮换池默认在 10 分钟窗口内共享最多 10 次探测机会，增加 URL 不会放大失败重试预算；时长和次数可在「冷却与重试」修改，成功后按模板续采时间重新开启预算。临期有效桶优先续采，同类桶轮流分配机会；每连续 3 次临期续采会为缺失桶保留一次机会，避免前面的坏桶让其他桶一直等待。成功后按模板实际签发时间和有效期安排续采。提前续采可设置为 10、20 等整数分钟，必须小于模板有效期；`renew_before_minutes=0` 保留自动策略：提前 5 分钟，短有效期则提前有效期的四分之一。调整该设置会重新安排成功出口的续采，不重置失败出口的冷却。升级前保存的旧冷却没有成功标记，会先按原期限结束一次。
...
在 **Codex Turn State → 采集范围** 的账号列表工具栏点击 **一键检测降智**，即可打开专用弹窗，不会跳转到模型测试页。按钮用于进入多题诊断，结果只评价每一道题，不会给账号贴上“降智／正常”的整体结论。

弹窗默认继承当前 State 勾选的账号，账号和题目均可多选；此处改选不修改采集草稿，也不会启用停用账号。模型默认使用已保存列表中的第一个，可手动填写。默认执行糖果保证题（自动检查最终答案 21）；也可勾选鹈鹕骑自行车 SVG 动画题。展开题目可查看原文、验收标准、预期答案与参考来源。SVG 题始终标为待人工评估，以隔离图片方式播放动画。

请求数为 **账号数 × 题目数**，一批最多 **50 次**，超限需减少选择。每项串行执行一次，失败后继续下一项；可查看本题通过、本题未通过、请求失败或待人工评估，并展开原文回答、断言、模型声明和实际代理通路。关闭弹窗、点击停止、离开 State 或退出登录后不再发送后续项，已发送的一项尽力收尾。结果仅在当前页面内存中保留；刷新不会在服务器上继续这份检测队列。

检测会消耗账号的上游额度，使用**已保存的 State 设置与账号代理 → 全局代理 → 显式直连**通路，不使用采集代理池，不会自动采票、修复 State、切换业务路由或产生下游 API Key 账单。自动采集保持运行，无需为检测停采；检测次数不计入采集小时预算，两者仍共享账号实际的上游额度。只有账号索引、Codex OAuth 类型及完整凭证标识均精确匹配时才允许检测，失效或身份不明确的条目显示原因，不会按名字猜号或静默换号。固定题通过不证明模型身份或整体能力，建议结合多类题目与已知正常账号人工对照。
...
在 **Codex Turn State → 冷却与重试** 修改并点击「保存所有设置」。新安装及旧配置缺少这些字段时，默认行为保持不变：

| 页面设置 | 配置字段 | 默认值 | 可设置范围 |
| --- | --- | --- | --- |
| 静态出口冷却 | `probe_static_cooldown_minutes` | 55 分钟 | 0 关闭；1–1440 整数分钟 |
| 轮换池重试窗口 | `probe_rotating_cooldown_minutes` | 10 分钟 | 0 关闭窗口限制；1–1440 整数分钟 |
| 账号暂停 | `probe_account_cooldown_minutes` | 10 分钟 | 0 关闭；1–1440 整数分钟 |
| 轮换池尝试次数上限 | `probe_rotating_max_attempts` | 10 次 | 0 不限次数；1–100 整数次 |

启用对应限制时，静态出口按「账号 × 模型 × 代理」分别冷却，空代理池使用的直连也遵守此规则；每次探测前预留冷却，失败不会立即重试同一出口。轮换池按「账号 × 模型」共享次数，从窗口首次尝试开始计时；用完次数后等到该窗口结束，增加 URL 不会扩大预算。成功采集按模板续采时间重新安排，避免旧失败窗口拖延正常续采。

HTTP 401、403、429 或无法读取有效 OAuth 凭证，会按「账号暂停」设置暂停该账号全部模型的采集，默认 10 分钟，设为 0 则不施加这类失败暂停。403 可能来自账号权限，也可能来自出口限制，单看状态码不能断定账号失效。只要账号仍在暂停，即使池里还有数百条未尝试的代理，也会显示「账号等待中」；暂停结束后仍需满足该出口的冷却规则。日志显示本次实际暂停分钟数、冷却已关闭或轮换尝试上限，修改设置不会改写历史日志。等待时的下次检查时间是调度检查时间，并不保证届时一定发出请求。

保存正数时长后，新时长用于后续创建的冷却或窗口，**已有截止时间和已用次数不会自动重置**。新的正数尝试上限按当前窗口已用次数立即判断，例如已用 10 次，将上限调到 15 后还剩 5 次；调到 5 后则等待现有窗口结束。设为 0 后，对应的失败等待会在下一次调度时被跳过，无需再手动清除；有效模板的正常续采等待仍然保留。重新开启限制时，仍未结束且未被后续探测替换的旧窗口会继续参与判断。在线采集器会在下一次心跳读取新设置，无需重新开启。缩短正数时长后若要解除现有冷却，可使用「清除冷却」；模板、小时预算和手动停止状态仍会保留。

**持续轮询代理：** 点击「取消冷却与次数限制」，会将以上四项及「每小时探测上限」填为 0；点击「保存全部设置」后生效。也可手动填 0。此按钮只修改草稿，不会启动采集、删除代理或清空模板；尚未开启的采集需再点「开始」。轮换窗口或轮换次数任一为 0，都不会因为该池尝试次数耗尽而等待；每小时上限若仍为正数，仍会等待小时额度恢复。

关闭限制后，采集失败会继续按代理池顺序轮询，走完回到开头；取得有效模板后仍等到提前续采时间再采集。每个账号同一时间仍只探测一个桶，同一账号两次探测之间保留至少约 2 秒的调度间隔，单次 25 秒超时。自动丢弃代理、最少保留数量仍按各自设置执行。持续尝试会消耗上游额度和代理流量；关闭插件冷却并不能恢复无效账号或解除上游自身的限制。

账号暂停的默认值与参考项目的 [10 分钟定义](https://github.com/arden-aaai/cpa-plugin-codex-turn-state/blob/13573a6c86449d1cab3f76ed9e1ae5511bcbb4aa/go/probe_runner.go#L174-L180)及 [401/403/429 处理](https://github.com/arden-aaai/cpa-plugin-codex-turn-state/blob/13573a6c86449d1cab3f76ed9e1ae5511bcbb4aa/go/probe_runner.go#L667-L680)对齐。升级前已保存的 55 分钟暂停同样保留原截止时间，需要提前重新检查时可手动清除一次冷却。
...
每个池可单独测试连通性：只建立 CPA 服务器到代理 URL 中网关地址与端口的 **TCP 连接**，不调用系统 ping，不向代理发送凭据，不访问 OpenAI 或出口 IP 查询站点；连接建立后立即关闭。每条最多等待 5 秒（含域名解析），最多同时检测 10 条。结果显示进度、代理地址、网关 IP 和连接耗时，分别统计连通、失败、未确认。网关 IP 是本次 TCP 对端地址，**不是代理出口 IP**；多条不同会话共用同一网关时可能全部连通，但不代表会话认证、出口质量或采集一定成功。

地址格式错误、网关域名解析失败、连接拒绝或连接超时可通过「移除本次失败代理」从文本框移除，点击保存后才生效；失败仅描述这次检测，不判定永久失效。管理接口连接失败等未确认结果保留。实际 State 采集仍需向 Codex 发送请求，与此入口连通性检测分开。采集日志和实时进度显示当前代理序号/总数、完整配置地址（含代理用户名和密码），轮换池同时显示重试次数；这些内容仅通过管理员接口返回并禁止缓存。成功模板的持久化采集地址仍隐藏认证信息。
...
探测使用 Go 内置 HTTP 客户端，无需服务器安装 `curl`。采集器的一次管理请求可能同步完成多次探测：按并发探测数分成几条通道，不同账号同时探测、每个账号同一时间一个，约 12 秒内陆续开始新的探测；每次探测总超时为 25 秒；每次新建连接，读取响应头后关闭响应和连接，避免轮换代理因连接复用而继续使用同一出口。探测使用选定账号直接请求 Codex，不修改 CPA 的账号启用状态或全局代理；**会消耗上游额度，直连探测不计入 CPA 下游 API Key 的计费记录**。连续任务由独立的 `cpa-state-collector` 进程定时唤醒。插件只在宿主调用内同步处理探测，不在内部启动后台任务。浏览器只读服务器状态并提交启停意图；关闭页面和退出登录不会停止服务器任务。停止后不再开始新探测，在途的探测可以收尾。租约和在途检查保证重复采集器不会并发探测；自动运行时旧浏览器或手动探测入口会被拒绝。保存配置和清空模板仍会与在途探测协调。WebSocket 握手响应头不在当前 HTTP/SSE 学习接口中；可先通过探测取得模板。
