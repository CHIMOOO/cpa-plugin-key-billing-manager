...
| Cookie | 上游 Cookie 按账号只保存在内存，所选账号的任何上游响应（包括探测和 312）都会刷新；默认 240 秒内所选模型的请求都会带上，与是否注入模板无关。列表中第一个模型采到 292 后默认 30 秒再采一次，保持 Cookie 新鲜；刷新没拿到 292 时保留当前模板，按同样间隔换出口继续刷新，刷新遇到 312 不会自动丢弃代理 |
...
关闭限制后，采集失败会继续按代理池顺序轮询，走完回到开头；取得有效模板后仍等到提前续采时间再采集。探测保持串行，每次结束后保留至少约 2 秒的调度间隔及单次 25 秒超时。自动丢弃代理、最少保留数量仍按各自设置执行。持续尝试会消耗上游额度和代理流量；关闭插件冷却并不能恢复无效账号或解除上游自身的限制。
