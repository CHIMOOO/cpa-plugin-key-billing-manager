...
| 流量抓包 | 选择一个上游账号，实时查看它收到的请求头、请求体与响应；只在内存中保留，请求头凭证打码 |
...
「流量抓包」用于排查某个上游账号实际收到的请求。选择账号并开始监听后，页面每 1.5 秒拉取一次，列出被调度到该账号的请求，点开可看到路径、请求头、请求体，以及响应头和响应正文（流式响应按分块拼接实时增长）。重试到其他账号的请求会标注出来，不会把别的账号的响应算到这里。
...
- **打码**：`Authorization`、`Cookie`、`X-Api-Key` 以及名称含 key、token、secret、password、signature 的请求头值会被替换为 `[redacted]`。请求体和响应体按原样显示，可能包含对话内容，请只在需要排查时开启。
- **能看到什么**：显示的是插件钩子在选定账号之后看到的内容，即客户端路径、请求头和请求体；开启响应钩子后，还能看到协议转换后发往上游的请求体。CPA 在上游执行器内部添加的上游凭证和最终 HTTP 报文不经过插件，这里看不到。
- **响应需要注册响应钩子**：只抓请求时不增加额外开销。勾选「注册响应钩子以捕获响应」后，需要重启 CPA 或重新加载插件配置才生效；之后每个流式分块都会经过插件（未监听时只做一次快速判断就返回）。如果 Codex Turn State 已开启，响应钩子本来就已注册。该开关保存在 `<state_file>.traffic-capture.json`。
