...
1. 在左侧账号栏按 **Codex、Claude、Gemini、OpenAI 兼容或其他服务商**筛选，也可搜索账号并多选。点击整张账号卡片即可选中或取消，已停用账号会标注状态并允许勾选；不支持测试或身份不明确的条目不展示。「选择当前筛选」只选择当前可见且可测试的账号；切换分类或搜索不会清除已选项。一批最多 50 个账号，窄屏时账号栏显示在测试设置上方。
...
弹窗默认继承当前 State 勾选的账号，账号和题目均可多选；此处改选不修改采集草稿，也不会启用停用账号。模型默认使用已保存列表中的第一个，可手动填写。默认执行精确 JSON、约束推理、布尔表达式三道客观题；也可勾选代码审查、鹈鹕 SVG、糖果保证和交换追踪，共七道预置题。展开题目可查看原文、验收标准、预期答案与参考来源。代码和 SVG 题始终标为待人工评估，代码不会执行，SVG 使用受限静态预览。
